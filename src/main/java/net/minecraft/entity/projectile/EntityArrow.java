                ...
                if (movingobjectposition.entityHit != null) {
                    ...
                    if (movingobjectposition.entityHit.attackEntityFrom(damagesource, (float)l)) {
                        if (movingobjectposition.entityHit instanceof EntityLivingBase entitylivingbase) {
                            ...
                            if (this.shootingEntity instanceof EntityLivingBase base) {
                                EnchantmentHelper.applyThornEnchantments(entitylivingbase, this.shootingEntity);
                                EnchantmentHelper.applyArthropodEnchantments(base, entitylivingbase);
                            }

                            if (this.shootingEntity != null && movingobjectposition.entityHit != this.shootingEntity && movingobjectposition.entityHit instanceof EntityPlayer && this.shootingEntity instanceof EntityPlayerMP mP) {
                                mP.playerNetServerHandler.sendPacket(new S2BPacketChangeGameState(6, 0.0F));
                            }
                        }
