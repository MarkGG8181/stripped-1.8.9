...
public abstract class EntityLivingBase extends Entity {
    ...
    protected EntityLivingBase(World worldIn) {
        super(worldIn);
        this.applyEntityAttributes();
        this.setHealth(this.getMaxHealth());
        this.preventEntitySpawning = true;
        this.randomUnused1 = (float)((ThreadLocalRandom.current().nextDouble() + 1.0D) * 0.009999999776482582D);
        this.setPosition(this.posX, this.posY, this.posZ);
        this.randomUnused2 = (float)ThreadLocalRandom.current().nextDouble() * 12398.0F;
        this.rotationYaw = (float)(ThreadLocalRandom.current().nextDouble() * Math.PI * 2.0D);
            ...
            if (this.getHealth() <= 0.0F) {
                ...
                if (flag) {
                    ...
                    if (entity != null) {
                        ...
                        for (d0 = entity.posZ - this.posZ; d1 * d1 + d0 * d0 < 1.0E-4D; d0 = (ThreadLocalRandom.current().nextDouble() - ThreadLocalRandom.current().nextDouble()) * 0.01D) {
                            d1 = (ThreadLocalRandom.current().nextDouble() - ThreadLocalRandom.current().nextDouble()) * 0.01D;
                        }
                        ...
                        this.attackedAtYaw = (float)((int)(ThreadLocalRandom.current().nextDouble() * 2.0D) * 180);
                    }
                }
    ...
    public void swingItem() {
        if (!this.isSwingInProgress || this.swingProgressInt >= this.getArmSwingAnimationEnd() / 2 || this.swingProgressInt < 0) {
            ...
            if (this.worldObj instanceof WorldServer server) {
                server.getEntityTracker().sendToAllTrackingEntity(this, new S0BPacketAnimation(this, 0));
            }
        }
    }
    ...
    public void moveEntityWithHeading(float strafe, float forward) {
        if (this.isServerWorld()) {
            if (!this.isInWater() || this instanceof EntityPlayer player1 && player1.capabilities.isFlying) {
                if (!this.isInLava() || this instanceof EntityPlayer player && player.capabilities.isFlying) {
                    float f4 = 0.91F;
