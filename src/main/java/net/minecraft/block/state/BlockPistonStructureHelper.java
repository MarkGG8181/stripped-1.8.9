...
public class BlockPistonStructureHelper
{
    ...
    public boolean canMove()
    {
        ...
        else if (!this.func177251A(this.blockToMove))
        {
            return false;
        }
        else
        {
            for (int i = 0; i < this.toMove.size(); i++)
            {
                ...
                if (this.world.getBlockState(blockpos).getBlock() == Blocks.slime_block && !this.func177250B(blockpos))
                {
                    return false;
                }
            }
    ...
    private boolean func177251A(BlockPos origin)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                while (true)
                {
                    ...
                    if (k > -1)
                    {
                        this.func177255A(i1, k);

                        for (int l = 0; l <= k + i1; l++)
                        {
                            BlockPos blockpos2 = (BlockPos)this.toMove.get(l);

                            if (this.world.getBlockState(blockpos2).getBlock() == Blocks.slime_block && !this.func177250B(blockpos2))
                            {
                                return false;
                            }
                        }
    ...
    private void func177255A(int p_177255_1_, int p_177255_2_)
    {
    ...
    private boolean func177250B(BlockPos p_177250_1_)
    {
        for (EnumFacing enumfacing : EnumFacing.values())
        {
            if (enumfacing.getAxis() != this.moveDirection.getAxis() && !this.func177251A(p_177250_1_.offset(enumfacing)))
            {
                return false;
            }
        }
