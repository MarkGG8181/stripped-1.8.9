...
public class WorldVertexBufferUploader
{
    ...
    public void draw(WorldRenderer p_181679_1_)
    {
        if (p_181679_1_.getVertexCount() > 0)
        {
            ...
            if (this.vboId == 0)
            {
                this.vboId = OpenGlHelper.glGenBuffers();
            }

            OpenGlHelper.glBindBuffer(OpenGlHelper.GL_ARRAY_BUFFER, this.vboId);
            ...
            for (int j = 0; j < list.size(); j++)
            {
                ...
                long offset = vertexformat.getOffset(j);

                switch (vertexformatelement$enumusage)
                {
                    case POSITION:
                        GL11.glVertexPointer(vertexformatelement.getElementCount(), k, i, offset);
                        GL11.glEnableClientState(GL11.GL_VERTEX_ARRAY);
                        break;
                    case UV:
                        OpenGlHelper.setClientActiveTexture(OpenGlHelper.defaultTexUnit + l);
                        GL11.glTexCoordPointer(vertexformatelement.getElementCount(), k, i, offset);
                        GL11.glEnableClientState(GL11.GL_TEXTURE_COORD_ARRAY);
                        OpenGlHelper.setClientActiveTexture(OpenGlHelper.defaultTexUnit);
                        break;
                    case COLOR:
                        GL11.glColorPointer(vertexformatelement.getElementCount(), k, i, offset);
                        GL11.glEnableClientState(GL11.GL_COLOR_ARRAY);
                        break;
                    case NORMAL:
                        GL11.glNormalPointer(k, i, offset);
