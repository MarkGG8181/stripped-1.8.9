...
public class GuiCustomizeWorldScreen extends GuiScreen implements GuiSlider.FormatHelper, GuiPageButtonList.GuiResponder
{
    ...
    public void func_175319_a(int p_175319_1_, String p_175319_2_)
    {
        ...
        {
            case 132 -> this.settings.mainNoiseScaleX = MathHelper.clamp_float(f, 1.0F, 5000.0F);

            case 133 -> this.settings.mainNoiseScaleY = MathHelper.clamp_float(f, 1.0F, 5000.0F);

            case 134 -> this.settings.mainNoiseScaleZ = MathHelper.clamp_float(f, 1.0F, 5000.0F);

            case 135 -> this.settings.depthNoiseScaleX = MathHelper.clamp_float(f, 1.0F, 2000.0F);

            case 136 -> this.settings.depthNoiseScaleZ = MathHelper.clamp_float(f, 1.0F, 2000.0F);

            case 137 -> this.settings.depthNoiseScaleExponent = MathHelper.clamp_float(f, 0.01F, 20.0F);

            case 138 -> this.settings.baseSize = MathHelper.clamp_float(f, 1.0F, 25.0F);

            case 139 -> this.settings.coordinateScale = MathHelper.clamp_float(f, 1.0F, 6000.0F);

            case 140 -> this.settings.heightScale = MathHelper.clamp_float(f, 1.0F, 6000.0F);

            case 141 -> this.settings.stretchY = MathHelper.clamp_float(f, 0.01F, 50.0F);

            case 142 -> this.settings.upperLimitScale = MathHelper.clamp_float(f, 1.0F, 5000.0F);

            case 143 -> this.settings.lowerLimitScale = MathHelper.clamp_float(f, 1.0F, 5000.0F);

            case 144 -> this.settings.biomeDepthWeight = MathHelper.clamp_float(f, 1.0F, 20.0F);

            case 145 -> this.settings.biomeDepthOffset = MathHelper.clamp_float(f, 0.0F, 20.0F);

            case 146 -> this.settings.biomeScaleWeight = MathHelper.clamp_float(f, 1.0F, 20.0F);

            case 147 -> this.settings.biomeScaleOffset = MathHelper.clamp_float(f, 0.0F, 20.0F);
            default -> 0.0F;
        };
    ...
    private String func_175330_b(int p_175330_1_, float p_175330_2_)
    {
        switch (p_175330_1_)
        {
            ...
            case 142:
            case 143:
                return "%5.3f".formatted(new Object[]{Float.valueOf(p_175330_2_)});
                ...
                case 301:
                    for (int i = 0; i < this.list.getSize(); i++)
                    {
                        ...
                        if (gui instanceof GuiButton guibutton)
                        {
                            ...
                            if (guibutton instanceof GuiSlider slider)
                            {
                                float f = slider.func_175217_d() * (0.75F + this.random.nextFloat() * 0.5F) + (this.random.nextFloat() * 0.1F - 0.05F);
                                slider.func_175219_a(MathHelper.clamp_float(f, 0.0F, 1.0F));
                            }
                            else if (guibutton instanceof GuiListButton listButton)
                            {
                                listButton.func_175212_b(this.random.nextBoolean());
                            }
                        }
                        ...
                        if (gui1 instanceof GuiButton guibutton1)
                        {
                            ...
                            if (guibutton1 instanceof GuiSlider slider)
                            {
                                float f1 = slider.func_175217_d() * (0.75F + this.random.nextFloat() * 0.5F) + (this.random.nextFloat() * 0.1F - 0.05F);
                                slider.func_175219_a(MathHelper.clamp_float(f1, 0.0F, 1.0F));
                            }
                            else if (guibutton1 instanceof GuiListButton listButton)
                            {
                                listButton.func_175212_b(this.random.nextBoolean());
                            }
                        }
