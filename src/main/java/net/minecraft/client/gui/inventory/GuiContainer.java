...
public abstract class GuiContainer extends GuiScreen {
    ...
    private float heldItemX, heldItemY;
    private boolean controllerItemHeld = false;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i1 = 0; i1 < this.inventorySlots.inventorySlots.size(); i1++) {
            ...
            if (this.isMouseOverSlot(slot, mouseX, mouseY) && slot.canBeHovered() && !controllerMode) {
                this.theSlot = slot;
                GlStateManager.disableLighting();
                GlStateManager.disableDepth();
        ...
        if (controllerMode && controllerSelectedSlot != null) {
        ...
        if (itemstack != null) {
            ...
            if (controllerMode && Controller.isConnected() && controllerSelectedSlot != null && !Mouse.isGrabbed()) {
                int slotX = controllerSelectedSlot.xDisplayPosition;
                int slotY = controllerSelectedSlot.yDisplayPosition;

                this.drawItemStack(itemstack, slotX, slotY, s);
            } else {
    ...
    protected void controllerButtonPressed(ControllerButton button) {
        ...
        if (button == ControllerButton.A) {
            ...
            if (heldStack == null && controllerSelectedSlot.getHasStack()) {
                //Pick up full stack (like left mouse click)
                this.handleMouseClick(controllerSelectedSlot, controllerSelectedSlot.slotNumber, 0, 0);
            } else if (heldStack != null) {
                //Place held stack down
                this.handleMouseClick(controllerSelectedSlot, controllerSelectedSlot.slotNumber, 0, 0);
            }
            return;
        }
        ...
        if (button == ControllerButton.X) {
            if (controllerSelectedSlot.getHasStack()) {
                this.handleMouseClick(controllerSelectedSlot, controllerSelectedSlot.slotNumber, 1, 0);
            }
            return;
        }
    ...
    public void updateScreen() {
        ...
        if (controllerMode && System.currentTimeMillis() - lastInputTime > 3000) {
